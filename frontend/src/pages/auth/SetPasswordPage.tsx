...
import type { SetPasswordFormErrors } from "../../types/auth";
...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    ...
    async function validateToken() {
      if (!token) {
        ...
        setTokenError("El enlace de activación es inválido o incompleto.");
        return;
      }
      ...
      try {
        ...
        setTokenStatus("invalid");
        setTokenError(
          "El enlace de activación ya expiró, fue usado o fue revocado."
        );
      } catch (error) {
        if (!isMounted) return;
        setTokenStatus("invalid");
        setTokenError(
          getErrorMessage(error, "No se pudo validar el enlace de activación.")
        );
      }
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await authService.setPasswordFromInvitation({
        token,
        newPassword: values.newPassword,
        confirmNewPassword: values.confirmNewPassword,
      });

      navigate("/admin/login?accountActivated=success", { replace: true });
    } catch (error) {
      const fieldErrors = getSetPasswordFieldErrors(error);
      ...
      setServerError(
        getErrorMessage(error, "No se pudo activar la cuenta")
      );
    } finally {
      setSubmitting(false);
    }
            ...
            {tokenStatus === "invalid" && (
              <>
                <FeedbackMessage
                  type="error"
                  message={
                    tokenError || "El enlace de activación no es válido."
                  }
                  layout="inline"
